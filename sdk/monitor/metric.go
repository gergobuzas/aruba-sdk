...
func GetMetrics(projectId, apiVersion, filter, sort, projection string, offset, limit int32) error {
	// API endpoint for listing projects
	endpoint := fmt.Sprintf("projects/%s/providers/Aruba.Insight/metrics", projectId)
	...
	queryParams.Add("api-version", fmt.Sprintf("%s", apiVersion))
